...
int main(int argc, char **argv) {
  ...
  while (n.ok()) {
    ...
    calibrate_cam.lookup_allMarkersToWorld(1, tf_MarkerToWorld);

    pick_pose.position.x = tf_MarkerToWorld.getOrigin()[0];
    pick_pose.position.y = tf_MarkerToWorld.getOrigin()[1];
    pick_pose.position.z = tf_MarkerToWorld.getOrigin()[2];
    pick_pose.orientation.x = tf_MarkerToWorld.getRotation()[0];    // This is the current calibrated pick pose 
    pick_pose.orientation.y = tf_MarkerToWorld.getRotation()[1];
    pick_pose.orientation.z = tf_MarkerToWorld.getRotation()[2];
    pick_pose.orientation.w = tf_MarkerToWorld.getRotation()[3];
